...
decrease of it. Falling BELOW [target - 5%] is a FAIL and blocks delivery until
the book is expanded into the band. OVERSHOOT IS ALLOWED (operator directive
2026-08-30): a book that renders above the band is accepted as it is, reported
with a note, and never trimmed to force a fit.
...
def evaluate(target, band_lo, band_hi, actual):
    ...
    # OVERSHOOT IS ALLOWED (operator directive 2026-08-30): a book that
    # renders ABOVE the target band is left as it is and PASSES; only an
    # under-count blocks. The band's high edge is reported for information.
    ok = actual >= band_lo
    rep = {"pass": ok, "target": target, "band": [band_lo, band_hi],
           "actual_pages": actual, "tolerance": TOL}
    if actual > band_hi:
        rep["note"] = ("book rendered %d pages, above the target band %d to %d: "
                       "ACCEPTED as is per the overshoot rule (2026-08-30); "
                       "never trim content to force a fit."
                       % (actual, band_lo, band_hi))
    if not ok:
        need = band_lo - actual
