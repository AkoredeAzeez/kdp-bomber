...
> **Framework integration:** the machine-executable projection of this niche's framework lives in `frameworks/AIRF-2.0-Activity-Edition/` (config, validation, phases, and deterministic ops in `genie_activity.py`). This SKILL.md remains the authoritative source; that folder never overrides it. At Phase 0 stamp `framework=AIRF 2.0 Activity Edition` into the `book_lock.md` classification line (global rule #16).
