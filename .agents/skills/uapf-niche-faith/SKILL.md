...
> **Framework integration:** the machine-executable projection of this niche's framework lives in `frameworks/AIRF-2.0/` (config, validation, phases, and deterministic ops in `genie_faith.py`). This SKILL.md remains the authoritative source; that folder never overrides it. At Phase 0 stamp `framework=AIRF 2.0` into the `book_lock.md` classification line (global rule #16).
...
- Chapter or part opener images: one image per part opener at ANY page count (auto-generate per global Rule 11 / AIRF 2.0 mandatory image rule; page count never gates generation). Faith imagery must be spiritually appropriate for the declared tradition, non-stereotyping, and never depicting actual scriptural figures in photorealistic form (illustration style preferred for sacred figures).
...
- [ ] All images generated and inserted (auto-rule applies at any page count)
