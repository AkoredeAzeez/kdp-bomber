...
def _find_root():
    """Genie root = nearest ancestor (including this folder) holding .agents/skills.
    Works for the skill copy (.agents/skills/uapf-automation-calendar/) and the
    root-level copy alike; the old fixed 4-levels-up walk pointed OUTSIDE the
    install when run from the root and wrote state into the user's home folder."""
    here = os.path.dirname(os.path.abspath(__file__))
    d = here
    for _ in range(6):
        if os.path.isdir(os.path.join(d, ".agents", "skills")):
            return d
        d = os.path.dirname(d)
